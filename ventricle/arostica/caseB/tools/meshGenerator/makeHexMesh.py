...
# CORRECTED-BASE VARIANT (Aróstica Case B geometry control).
#
# The only difference from conforming_hexmesh.py is the basal truncation.
# The paper (Eqs. 10-11) truncates the endocardial and epicardial ellipsoids
# at DIFFERENT parametric angles,
#
#     endocardium  mu in [-pi, -arccos(5/17)]   ->  x = 0.090*5/17 = 0.02647059
#     epicardium   mu in [-pi, -arccos(5/20)]   ->  x = 0.097*5/20 = 0.02425000
#
# so the base is a sloped (ruled) surface with a 2.2206 mm axial drop across
# the wall, an inclination of 12.5267 degrees from the radial direction. The
# original script instead imposes a single plane x = base_x = 0.025 on every
# transmural layer.
#
# Here cos(mu_truncation) is interpolated linearly in the transmural
# coordinate, exactly as the paper interpolates r_long and r_short in (16),
# so the truncation of every intermediate surface is the natural continuation
# of the two prescribed rims. This reproduces both rims exactly and leaves the
# endocardial surface, the epicardial surface and the apex untouched.
MONO = {
    "r_short_endo": 0.025,
    "r_short_epi": 0.035,
    "r_long_endo": 0.090,
    "r_long_epi": 0.097,
    "cos_mu_endo": 5.0 / 17.0,
    "cos_mu_epi": 5.0 / 20.0,
}


def mono_base_x(transmural):
    """Axial position of the paper's truncation for a transmural layer."""
    a = MONO["r_long_endo"] + transmural * (MONO["r_long_epi"] - MONO["r_long_endo"])
    cos_mu = MONO["cos_mu_endo"] + transmural * (MONO["cos_mu_epi"] - MONO["cos_mu_endo"])
    return a * cos_mu
...
def mono_point(disk_y, disk_z, transmural):
    ...
    x = -a + (mono_base_x(transmural) + a) * radial_parameter * radial_parameter
...
def build_monoventricle(output: Path, surface_divisions: int, longitudinal_divisions: int, transmural_layers: int):
    ...
    # Base error is now measured against the paper's ruled truncation surface
    # rather than a single plane: for each basal point recover its transmural
    # layer from the radius and compare with the analytic truncation.
    base_errors = []
    for key, patch in boundary_keys.items():
        if patch != "base":
            continue
        for index in key:
            x, y, z = points[index]
            ...
            for _ in range(80):
                mid = 0.5 * (lo + hi)
                b = MONO["r_short_endo"] + mid * (MONO["r_short_epi"] - MONO["r_short_endo"])
                c = MONO["cos_mu_endo"] + mid * (MONO["cos_mu_epi"] - MONO["cos_mu_endo"])
                if b * math.sqrt(max(0.0, 1.0 - c * c)) < r:
                    lo = mid
                else:
                    hi = mid
            base_errors.append(abs(x - mono_base_x(0.5 * (lo + hi))))
    surface_errors["base"] = {
        "max_ruled_surface_error_m": max(base_errors),
        "mean_ruled_surface_error_m": sum(base_errors) / len(base_errors),
    }

    summary = {
        "case": "monoventricle",
        "method": "mapped five-block ellipsoidal shell; compact concentric cap and regular longitudinal rings",
        "geometry_source": "Aróstica paper nested truncated ellipsoids, PAPER-FAITHFUL SLOPED BASE",
